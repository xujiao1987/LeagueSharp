...
namespace LightningRyze
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
			...
			Config = new Menu("瑞兹", "Twilight Ryze", true);
			var targetSelectorMenu = new Menu("目标选择", "Target Selector");
			SimpleTs.AddToMenu(targetSelectorMenu);
			Config.AddSubMenu(targetSelectorMenu);
			
            var orbwalkerMenu = new Menu("LX-走砍", "Orbalker");
            LXOrbwalker.AddToMenu(orbwalkerMenu);
            Config.AddSubMenu(orbwalkerMenu);
			
			Config.AddSubMenu(new Menu("连招", "Combo"));
			Config.SubMenu("Combo").AddItem(new MenuItem("ComboActive", "热键").SetValue(new KeyBind(32, KeyBindType.Press)));
			Config.SubMenu("Combo").AddItem(new MenuItem("TypeCombo", "").SetValue(new StringList(new[] {"混合","爆发","持续"},0)));
            ...
            Config.SubMenu("Harass").AddItem(new MenuItem("HarassActive", "热键").SetValue(new KeyBind("N".ToCharArray()[0], KeyBindType.Press)));
			Config.SubMenu("Harass").AddItem(new MenuItem("HQ", "使用Q").SetValue(true));
			Config.SubMenu("Harass").AddItem(new MenuItem("HW", "使用W").SetValue(true));
            Config.SubMenu("Harass").AddItem(new MenuItem("HE", "使用E").SetValue(true));
			...
			Config.SubMenu("Farm").AddItem(new MenuItem("FQ", "使用Q").SetValue(true));
			Config.SubMenu("Farm").AddItem(new MenuItem("FW", "使用W").SetValue(true));
            Config.SubMenu("Farm").AddItem(new MenuItem("FE", "使用E").SetValue(true));
            Config.SubMenu("Farm").AddItem(new MenuItem("FR", "使用R").SetValue(true));

            Config.AddSubMenu(new Menu("杂项", "Exploit"));
            Config.SubMenu("Exploit").AddItem(new MenuItem("UsePacket", "封包").SetValue(true));
            Config.SubMenu("Exploit").AddItem(new MenuItem("tearStack", "Q+W叠2次眼泪").SetValue(new KeyBind("J".ToCharArray()[0], KeyBindType.Press)));

            Config.AddSubMenu(new Menu("清野", "JungleFarm"));
            Config.SubMenu("JungleFarm").AddItem(new MenuItem("JungActive", "热键").SetValue(new KeyBind("V".ToCharArray()[0], KeyBindType.Press)));
			Config.SubMenu("JungleFarm").AddItem(new MenuItem("JQ", "使用Q").SetValue(true));
			Config.SubMenu("JungleFarm").AddItem(new MenuItem("JW", "使用W").SetValue(true));
            Config.SubMenu("JungleFarm").AddItem(new MenuItem("JE", "使用E").SetValue(true));
            Config.SubMenu("JungleFarm").AddItem(new MenuItem("JR", "使用R").SetValue(true));
            
            Config.AddSubMenu(new Menu("抢头", "KillSteal"));
            Config.SubMenu("KillSteal").AddItem(new MenuItem("KillSteal", "抢头").SetValue(true));
            Config.SubMenu("KillSteal").AddItem(new MenuItem("AutoIgnite", "使用点燃").SetValue(true));
            
            Config.AddSubMenu(new Menu("其他", "Extra"));
            Config.SubMenu("Extra").AddItem(new MenuItem("UseSera", "使用大天使").SetValue(true));
            Config.SubMenu("Extra").AddItem(new MenuItem("HP", "血量小于").SetValue(new Slider(20, 100, 0)));
            Config.SubMenu("Extra").AddItem(new MenuItem("UseWGap", "使用W防突").SetValue(true));
            Config.SubMenu("Extra").AddItem(new MenuItem("AutoPoke", "骚扰 (锁定)").SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Toggle)));
            Config.SubMenu("Extra").AddItem(new MenuItem("ManaFreeze", "蓝量控制").SetValue(new Slider(40, 1, 100)));
            Config.SubMenu("Extra").AddItem(new MenuItem("WInterruptSpell", "使用W打断").SetValue(true));
            
            Config.AddSubMenu(new Menu("地图", "MapHack"));
            Config.SubMenu("MapHack").AddItem(new MenuItem("TextColorMH", "文本颜色").SetValue(new Circle(true, Color.FromArgb(255, 255, 255, 0))));
            Config.SubMenu("MapHack").AddItem(new MenuItem("OutlineColorMH", "颜色").SetValue(new Circle(true, Color.FromArgb(255, 0, 0, 0))));
            Config.SubMenu("MapHack").AddItem(new MenuItem("MapHack", "打开").SetValue(true));
			...
			Config.SubMenu("Drawings").AddItem(new MenuItem("WERange", "W+E范围").SetValue(new Circle(false, Color.FromArgb(255, 255, 255, 255))));
            Config.SubMenu("Drawings").AddItem(new MenuItem("drawDamage", "显示伤害").SetValue(true));
			
			Config.AddToMainMenu();

            Game.PrintChat("TAC Ryze loaded!");
            Game.PrintChat("============================");
