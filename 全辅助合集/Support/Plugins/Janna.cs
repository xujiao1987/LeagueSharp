...
namespace Support.Plugins
{
    public class Janna : PluginBase
    {
        ...
        {
            config.AddBool("Harass.W", "浣跨敤 W", true);
        }

        public override void MiscMenu(Menu config)
        {
            config.AddBool("Misc.E.Tower", "鍦ㄥ涓嬩娇鐢ㄣ劎E", true);

            // build aa menu
            var aa = config.AddSubMenu(new Menu("浣跨敤 E 鍐嶆敾鍑汇劎", "Misc.E.AA.Menu"));
            ...
            // build spell menu
            var dmg = config.AddSubMenu(new Menu("浣跨敤 E 鐨勬硶鏈劎", "Misc.E.Spell.Menu"));
            foreach (
                var spell in
