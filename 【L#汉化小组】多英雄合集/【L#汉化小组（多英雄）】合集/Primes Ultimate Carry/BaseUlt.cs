...
namespace Primes_Ultimate_Carry
{
	class BaseUlt
	{
		...
		{
			menu.AddItem(new MenuItem("baseult_sep0", "====== 基地大招"));
			menu.AddItem(new MenuItem("showRecalls", "= 显示回城").SetValue(true));
			menu.AddItem(new MenuItem("baseUlt", "= 基地大招").SetValue(true));
			menu.AddItem(new MenuItem("extraDelay", "= 额外的延迟ㄧ").SetValue(new Slider(0, -2000, 2000)));
			menu.AddItem(
				new MenuItem("panicKey", "= 不使用基地大招按键ㄧ").SetValue(new KeyBind(32, KeyBindType.Press)));
			menu.AddItem(
				new MenuItem("regardlessKey", "= 没有时间限制按键").SetValue(new KeyBind(17, KeyBindType.Press)));
			menu.AddItem(new MenuItem("debugMode", "= 调试").SetValue(false).DontSave());

			var teamUlt = menu.AddSubMenu(new Menu("使用基地大招的队友ㄧ", "TeamUlt"));
