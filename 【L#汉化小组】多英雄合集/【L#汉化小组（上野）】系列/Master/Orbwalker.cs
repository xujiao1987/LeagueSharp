    ...
    class M_Orbwalker
    {
        ...
        public static void AddToMenu(Menu menu)
        {
            ...
            var OWMenu = new Menu("走砍", "OW");
            {
                ...
                {
                    DrawMenu.AddItem(new MenuItem("OW_Draw_AARange", "平A范围").SetValue(new Circle(true, Color.FloralWhite)));
                    DrawMenu.AddItem(new MenuItem("OW_Draw_AARangeEnemy", "敌人平A范围").SetValue(new Circle(true, Color.Pink)));
                    DrawMenu.AddItem(new MenuItem("OW_Draw_HoldZone", "控制区域").SetValue(new Circle(true, Color.FloralWhite)));
                    DrawMenu.AddItem(new MenuItem("OW_Draw_LastHit", "显示补兵").SetValue(new Circle(true, Color.Lime)));
                    DrawMenu.AddItem(new MenuItem("OW_Draw_NearKill", "附近可杀小兵").SetValue(new Circle(true, Color.Gold)));
                    OWMenu.AddSubMenu(DrawMenu);
                }
                var MiscMenu = new Menu("杂项", "Misc");
                {
                    MiscMenu.AddItem(new MenuItem("OW_Misc_HoldZone", "控制区域").SetValue(new Slider(50, 100, 0)));
                    MiscMenu.AddItem(new MenuItem("OW_Misc_FarmDelay", "补兵延迟").SetValue(new Slider(0, 200, 0)));
                    MiscMenu.AddItem(new MenuItem("OW_Misc_ExtraWindUp", "额外时间").SetValue(new Slider(80, 200, 0)));
                    MiscMenu.AddItem(new MenuItem("OW_Misc_AutoWindUp", "自动设置").SetValue(true));
                    MiscMenu.AddItem(new MenuItem("OW_Misc_PriorityUnit", "优先").SetValue(new StringList(new[] { "小兵", "英雄" })));
                    MiscMenu.AddItem(new MenuItem("OW_Misc_Humanizer", "延迟").SetValue(new Slider(80, 200, 15)));
                    MiscMenu.AddItem(new MenuItem("OW_Misc_MeleePrediction", "移动预判").SetValue(false));
                ...
                {
                    var ComboMenu = new Menu("连招", "Mode_Combo");
                    {
                    ...
                    var HarassMenu = new Menu("骚扰", "Mode_Harass");
                    {
                    ...
                    var ClearMenu = new Menu("清线", "Mode_Clear");
                    {
                        ClearMenu.AddItem(new MenuItem("OW_Clear_Key", "热键").SetValue(new KeyBind("G".ToCharArray()[0], KeyBindType.Press)));
                    ...
                    var FreezeMenu = new Menu("控线", "Mode_Freeze");
                    {
                        FreezeMenu.AddItem(new MenuItem("OW_Freeze_Key", "热键").SetValue(new KeyBind("Y".ToCharArray()[0], KeyBindType.Press)));
                        FreezeMenu.AddItem(new MenuItem("OW_Freeze_Move", "移动").SetValue(true));
                        FreezeMenu.AddItem(new MenuItem("OW_Freeze_Attack", "攻击").SetValue(true));
                        ModeMenu.AddSubMenu(FreezeMenu);
                    }
                    var LastHitMenu = new Menu("补兵", "Mode_LastHit");
                    {
                    ...
                    var FleeMenu = new Menu("逃跑", "Mode_Flee");
                    {
                        FleeMenu.AddItem(new MenuItem("OW_Flee_Key", "热键").SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Press)));
