...
namespace MasterCommon
{
    class M_TargetSelector
    {
        ...
        public M_TargetSelector(Menu menu, float range)
        {
            ...
            {
                TSMenu.AddItem(new MenuItem("TS_Mode", "模式").SetValue(new StringList(new[] { "优先", "最高AD", "最高AP", "最低攻击", "最低魔法", "最低血量", "最近", "鼠标附近" })));
                TSMenu.AddItem(new MenuItem("TS_Focus", "锁定目标").SetValue(true));
                TSMenu.AddItem(new MenuItem("TS_Draw", "显示锁定").SetValue(true));
                TSMenu.AddItem(new MenuItem("TS_Print", "显示新目标").SetValue(true));
                TSMenu.AddItem(new MenuItem("TS_AutoPrior", "自动排列优先级").SetValue(true)).ValueChanged += PriorityChanger;
