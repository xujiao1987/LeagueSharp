...
namespace xSLx_Orbwalker
{
	...
	public class xSLxOrbwalker
	{
		...
		public static void AddToMenu(Menu menu)
		{
			...
			menuDrawing.AddItem(new MenuItem("orb_Draw_AARange", "骞矨鑼冨洿").SetValue(new Circle(true, Color.FloralWhite)));
			menuDrawing.AddItem(new MenuItem("orb_Draw_AARange_Enemy", "鏁屼汉骞矨鑼冨洿").SetValue(new Circle(true, Color.Pink)));
			menuDrawing.AddItem(new MenuItem("orb_Draw_Holdzone", "鎺у埗鍖哄煙").SetValue(new Circle(true, Color.FloralWhite)));
			menuDrawing.AddItem(new MenuItem("orb_Draw_MinionHPBar", "灏忓叺琛€鏍紎").SetValue(new Circle(true, Color.Black)));
			menuDrawing.AddItem(new MenuItem("orb_Draw_MinionHPBar_thickness", "瀵嗗害").SetValue(new Slider(1, 1, 3)));
			menuDrawing.AddItem(new MenuItem("orb_Draw_hitbox", "鏄剧ず鍛戒腑").SetValue(new Circle(true, Color.FloralWhite)));
			menuDrawing.AddItem(new MenuItem("orb_Draw_Lasthit", "琛ュ叺").SetValue(new Circle(true, Color.Lime)));
			menuDrawing.AddItem(new MenuItem("orb_Draw_nearKill", "闄勮繎灏忓叺").SetValue(new Circle(true, Color.Gold)));
			menu.AddSubMenu(menuDrawing);

			var menuMisc = new Menu("鏉傞」", "orb_Misc");
			menuMisc.AddItem(new MenuItem("orb_Misc_Holdzone", "闃插畧").SetValue(new Slider(50, 100, 0)));
			menuMisc.AddItem(new MenuItem("orb_Misc_Farmdelay", "琛ュ叺寤惰繜").SetValue(new Slider(0, 200, 0)));
			menuMisc.AddItem(new MenuItem("orb_Misc_ExtraWindUp", "棰濆").SetValue(new Slider(80, 200, 0)));
			...
			menuMelee.AddItem(new MenuItem("orb_Melee_Prediction", "绉诲姩棰勬祴").SetValue(false));
			...
			var menuModes = new Menu("璧扮爫", "orb_Modes");
			{
				...
				modeCombo.AddItem(new MenuItem("Combo_attack", "琛ュ叺").SetValue(true));
				...
				modeHarass.AddItem(new MenuItem("Harass_attack", "琛ュ叺").SetValue(true));
				modeHarass.AddItem(new MenuItem("Harass_Lasthit", "Lasthit Minions").SetValue(true));
				menuModes.AddSubMenu(modeHarass);

				var modeLaneClear = new Menu("楠氭壈", "orb_Modes_LaneClear");
				...
				modeLaneClear.AddItem(new MenuItem("LaneClear_attack", "琛ュ叺").SetValue(true));
