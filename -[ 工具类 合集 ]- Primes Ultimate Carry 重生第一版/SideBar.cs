...
namespace Primes_Ultimate_Carry
{
	class SideBar
	{
		...
		internal static void AddtoMenu(Menu menu)
		{
			...
			var tempMenu = menu;
			tempMenu.AddItem(new MenuItem("sb_sep0", "====== Settings"));
			tempMenu.AddItem(new MenuItem("sb_show", "= Show Sidebar").SetValue(new KeyBind("S".ToCharArray()[0], KeyBindType.Press)));
